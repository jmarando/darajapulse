...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: uData, error: userErr } = await admin.auth.getUser(token);
    userId = uData?.user?.id ?? null;
    if (!userId) return json({ error: "unauthorized", detail: userErr?.message || "invalid or expired session" }, 401);
